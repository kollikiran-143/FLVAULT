...
public class BandhanServiceImpl implements BandhanService{
	...
	public BSInfo parseBandhan1(ParseBankStmtRequestDTO request) {
        ...
        try {
            ...
            bankStatementInfo.setNominee(CommonUtils.extractField(pdfText, "Joint\\s*Holder\\s*Names\\s*(.*)").replaceAll("\\s+", " "));
...
//        	e.printStackTrace();
		...
		if(txnRows != null && !txnRows.isEmpty()) {
			for (String[] row : txnRows) {
				...
				if(dateMatcher.find()) {
					...
					transaction.setValueDate(CommonUtils.dateFormatter(lineArr[1], dateFormat));
					String desc = lineArr[2];
					String[] lineArr3Split = lineArr[3].split("\\s");
					
					for (int i = 0; i < lineArr3Split.length; i++) {
						if(i==1) {
							transaction.setAmount(lineArr3Split[1]);
						}else if(i>1){
	...
	private List<Transaction> extractTransactionsBANDHAN_3(String pdfText, String accountNo) throws IOException {
        List<Transaction> transactions = new ArrayList<>();
        pdfText = pdfText.replaceAll("-This\\s*is\\s*computer\\s*generated\\s*statement[\\s\\S]*?\\s*W\\s*ebsite:www.bandhanbank.com", "")
        				 .replaceAll("page[\\s\\S]*?\\s*Statement\\s*of\\s*Account.*", "");
