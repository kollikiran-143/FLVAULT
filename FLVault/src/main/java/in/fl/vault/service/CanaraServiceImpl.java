	...
	public BSInfo parseCanara5(ParseBankStmtRequestDTO request) throws IOException, InterruptedException {
		...
		try {
			...
			String[] period = CommonUtils.extractMultiGroupArray(pdfText,
					"IFSC\\s*Code\\s*(CNRB\\w{7})[\\s\\S]*?From\\s*(\\d{2}\\s*[A-Za-z]{3}\\s*\\d{4}).*(\\d{2}\\s*[A-Za-z]{3}\\s*\\d{4})\\n\\s*Account\\s*Number\\s*(\\d*)");
			if (period != null) {
...
//			e.printStackTrace();
	...
	public List<Transaction> extractTransactionsCanara_1(String pdfText, String accountNo) throws IOException {
		...
		pdfText = pdfText.replaceAll("\\s*Statement[\\s\\S]*?Opening\\s*Balance.*", "").replaceAll(".*page[\\s\\S]*?Balance.*", "");
...
//						System.out.println("line---------->"+line);
				if (dateMatcher.find() && data.length == 8) {
					...
					// We are getting both credit and debit as 0.00 in first transaction
					if (transaction.getDebit().equalsIgnoreCase("") && transaction.getCredit().equalsIgnoreCase("")) {
						serialNoCount--;
					} else {
						transactions.add(transaction);
					}
