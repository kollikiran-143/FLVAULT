...
public class DBSServiceImpl implements DBSService {
	...
	public BSInfo parseDBS1(ParseBankStmtRequestDTO request) {
		...
		try {
			...
			List<Transaction> transactions = extractTransactionsDBS_1(text, accountNo, "dd-MMM-yyyy");
...
//    }
	...
	private List<Transaction> extractTransactionsDBS_1(String pdfText, String accountNo, String dateFormat) throws IOException {
		List<Transaction> transactions = new ArrayList<>();

		Pattern pattern = Pattern
				.compile("(\\d{2}-\\w{3}-\\d{4})\\s*(\\d{2}-\\w{3}-\\d{4})\\s+(.*?)\\s*([\\d,]+\\.\\d{2})(\\s+)([\\d,]+\\.\\d{2})([\\s\\S]+?)(?=(\\s*\\d{2}-\\w{3}-\\d{4})|(PRIVATE\\s*&\\s*))");
	...
	private List<Transaction> extractTransactionsDBS_2(String text, String accountNo) {
		List<Transaction> transactions = new ArrayList<>();
		String[] lines = text.split("\n");
		Pattern transactionPattern = Pattern.compile("(\\d{2}/\\d{2}/\\d{4})\\s*(\\d{2}/\\d{2}/\\d{4})\\s*(\\d*)\\s*(.*)\\s+([\\d,]*.\\d{2})\\s+([\\d,]*.\\d{2})");
		Pattern continuationPattern = Pattern.compile("^(.+)$");
