...
//			e.printStackTrace();
		...
		log.info("Exiting PSIBServiceImpl parsePSIB1: " + bankstatementInfo);
	...
	public BSInfo parsePSIB2(ParseBankStmtRequestDTO request) {
		...
		try {
			...
			bankstatementInfo.setAddress(CommonUtils.extractMultiLinesField(pdfText, "\\n(\\s*Customer\\s*Address[\\s\\S]*?)\\n\\s*Period", 100)
					.replaceAll("Customer", "").replaceAll("\\s+", " ").trim());
...
//			e.printStackTrace();
		...
		log.info("Exiting PSIBServiceImpl parsePSIB2: " + bankstatementInfo);
		long timeTaken = System.currentTimeMillis() - startTimeInMillis;
