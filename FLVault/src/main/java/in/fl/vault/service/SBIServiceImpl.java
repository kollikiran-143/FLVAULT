...
//        	e.printStackTrace();
        ...
        log.info("Exiting SBIServiceImpl parseSBI1: " + bankStatementInfo);
...
//			e.printStackTrace();
		...
		log.info("Exiting SBIServiceImpl parseSBI2: " + bankstatementInfo);
		long timeTaken = System.currentTimeMillis() - startTimeInMillis;
	...
	public BSInfo parseSBI3(ParseBankStmtRequestDTO request) throws IOException {
		...
		try {
			...
			String[] arrayOfStrings = CommonUtils.extractMultiGroupArray(text,
					"\\s+Statement\\s*of(.+)\\(A\\/c-(.+)\\)\\s+between\\s+(\\d+-[A-Za-z]+-\\d+)\\s+to\\s+(\\d+-[A-Za-z]+-\\d+)");
			...
			bankStatementInfo.setTransactions(
					extractTransactionsSBI_3(filePath, bankStatementInfo.getAccountNo(), txnDateFormat));
...
//			e.printStackTrace();
		...
		log.info("Exiting SBIServiceImpl parseSBI3: " + bankStatementInfo);
		long timeTaken = System.currentTimeMillis() - startTimeInMillis;
...
//        	e.printStackTrace();
        ...
        log.info("Exiting SBIServiceImpl SBI4: " + bankStatementInfo);
		long timeTaken = System.currentTimeMillis() - startTimeInMillis;
...
//        	e.printStackTrace();
        ...
        log.info("Exiting SBIServiceImpl SBI5: " + bankStatementInfo);
		long timeTaken = System.currentTimeMillis() - startTimeInMillis;
...
//        	e.printStackTrace();
        ...
        log.info("Exiting SBIServiceImpl, SBI6: " + bankStatementInfo);
		long timeTaken = System.currentTimeMillis() - startTimeInMillis;
	...
	public BSInfo parseSBI7(ParseBankStmtRequestDTO request) throws IOException {
		...
		try {
			...
			bankStatementInfo
					.setName(CommonUtils.extractField(text, "Account\\s*Name\\s*(.+)").replaceAll("\\s+", " "));
			bankStatementInfo.setAddress(
					CommonUtils.extractField(text, "Address\\s*(.+?)(?=Date)", Pattern.DOTALL).replaceAll("\\s+", " "));
			bankStatementInfo.setAccountNo(CommonUtils.extractField(text, "Account\\s*Number\\s*(\\d+)"));
			bankStatementInfo.setAccountType(
					CommonUtils.extractField(text, "Account\\s*Description\\s*(.+)").replaceAll("\\s+", " "));
			...
			String dateFormat = "dd MMM yyyy";
			String txnDateFormat = "dd MMM yyyy";
			bankStatementInfo.setStartDate(CommonUtils.dateFormatter(
					CommonUtils.extractField(text, "to\\s+(\\d+\\s+[A-Za-z]+\\s+\\d{4})").replaceAll("\\s+", " "),
					dateFormat));
			bankStatementInfo.setEnDate(CommonUtils.dateFormatter(CommonUtils
					.extractField(text, "Search\\s*for\\s+(\\d+\\s+[A-Za-z]+\\s+\\d{4})").replaceAll("\\s+", " "),
					dateFormat));
			bankStatementInfo.setTransactions(
					extractTransactionsSBI_7(filePath, bankStatementInfo.getAccountNo(), txnDateFormat));
		} catch (Exception e) {
//			e.printStackTrace();
			log.error("Error in SBIServiceImpl parseSBI7: " + e);
		}

		log.info("Exiting SBIServiceImpl, parseSBI7: " + bankStatementInfo);
		long timeTaken = System.currentTimeMillis() - startTimeInMillis;
	...
	public BSInfo parseSBI8(ParseBankStmtRequestDTO request) throws IOException {
		...
		try {
			...
			bankStatementInfo.setEnDate(CommonUtils.dateFormatter(CommonUtils
					...
					dateFormat));
			bankStatementInfo
					.setTransactions(extractTransactionsSBI_8(text, bankStatementInfo.getAccountNo(), txnDateFormat));
...
//			e.printStackTrace();
		...
		log.info("Exiting SBIServiceImpl, parseSBI8: " + bankStatementInfo);
		long timeTaken = System.currentTimeMillis() - startTimeInMillis;
	...
	public BSInfo parseSBI9(ParseBankStmtRequestDTO request) throws IOException {
		...
		try {
			...
			bankStatementInfo.setBranch(CommonUtils.extractField(text, "Branch\\s*(.*)").replaceAll("\\s+", " "));
			bankStatementInfo.setAddress(CommonUtils.extractField(text, "Address([\\s\\S]*?)Account")
					.replaceAll("\\n", " ").replaceAll("\\s+", " ").trim());
			bankStatementInfo
			...
			bankStatementInfo.setNominee(
					CommonUtils.extractField(text, "Nomination\\s*Registered\\s*(.*)").replaceAll("\\s+", " "));
...
//			e.printStackTrace();
		...
		log.info("Exiting SBIServiceImpl parseSBI9: " + bankStatementInfo);
		long timeTaken = System.currentTimeMillis() - startTimeInMillis;
	...
	public BSInfo parseSBI10(ParseBankStmtRequestDTO request){
		...
		try {
			...
			String[] dates = CommonUtils.extractMultiGroupArray(text,
					"Statement\\s*From\\s*:\\s*(\\d{2}\\s*\\w{3}\\s*\\d{4}).*?(\\d{2}\\s*\\w{3}\\s*\\d{4})");
			...
			bsInfo.setName(
					CommonUtils.extractMultiLinesField(text, "Name\\s*DoB.*([\\s\\S]*?)(?=\\s*SUMMARY)", 0, 17).trim());
			bsInfo.setPhone1(CommonUtils
					.extractMultiLinesField(text, "Name\\s*DoB.*([\\s\\S]*?)(?=\\s*SUMMARY)", 35, 11).trim());
			bsInfo.setDob(CommonUtils.extractMultiLinesField(text, "Name\\s*DoB.*([\\s\\S]*?)(?=\\s*SUMMARY)", 18, 18)
					.trim());
			bsInfo.setEmail(CommonUtils.extractMultiLinesField(text, "Name\\s*DoB.*([\\s\\S]*?)(?=\\s*SUMMARY)", 54, 27)
					.trim());
			bsInfo.setPan(CommonUtils.extractMultiLinesField(text, "Name\\s*DoB.*([\\s\\S]*?)(?=\\s*SUMMARY)", 89, 13)
					.trim());
			bsInfo.setAddress(CommonUtils
					.extractMultiLinesField(text, "Name\\s*DoB.*([\\s\\S]*?)(?=\\s*SUMMARY)", 101, 27).trim());
			bsInfo.setNominee(CommonUtils
					.extractMultiLinesField(text, "Name\\s*DoB.*([\\s\\S]*?)(?=\\s*SUMMARY)", 143, 15).trim());
...
//			e.printStackTrace();
		...
		log.info("Exiting SBIServiceImpl parseSBI10: " + bsInfo);
		long timeTaken = System.currentTimeMillis() - startTimeInMillis;
	...
	private List<Transaction> extractTransactionsSBI_1_4_6(String fileName, String accountNo, String dateFormat) throws IOException {
		...
		if(txnRows != null && !txnRows.isEmpty()) {
			...
			for (String[] row : txnRows) {
				...
				if (line.contains("CLOSING BALANCE") || line.contains("BROUGHT FORWARD") || 
						data.length != 7 || headerMatcher.find() || data[4].equalsIgnoreCase("debit"))
					continue;
	...
	private List<Transaction> extractTransactionsSBI_3(String fileName, String accountNo, String dateFormat)
			throws IOException {
		List<Transaction> transactions = new ArrayList<>();
		List<String[]> txnRows = CommonUtils.tabulaExtraction(fileName);
	...
	private List<Transaction> extractTransactionsSBI_7(String fileName, String accountNo, String dateFormat)
			throws IOException {
		List<Transaction> transactions = new ArrayList<>();
		List<String[]> txnRows = CommonUtils.tabulaExtraction(fileName);
	...
	private List<Transaction> extractTransactionsSBI_8(String pdfText, String accountNo, String dateFormat)
			throws IOException {
		List<Transaction> transactions = new ArrayList<>();
		...
		for (String eachLine : textInArray) {
			...
			String data[] = CommonUtils.extractMultiGroupArray(eachLine,
					"\\s*(\\d{2}\\s+\\w{3}\\s+\\d{4})\\s*([\\s\\S]+?)\\s+(\\S+)\\s+(\\S+)\\s+(\\S+)$");
			if (data != null) {
	...
	private List<Transaction> extractTransactionsSBI_9(String pdfText, String accountNo, String dateFormat)
			throws IOException {
		List<Transaction> transactions = new ArrayList<>();
	...
	private List<Transaction> extractTransactionsSBI_10(String fileName, String accountNo, BSInfo bsInfo)
			throws IOException {
		List<Transaction> transactions = new ArrayList<>();
		List<String[]> txnRows = CommonUtils.tabulaExtraction(fileName);
