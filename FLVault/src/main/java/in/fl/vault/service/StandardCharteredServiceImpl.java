...
//			e.printStackTrace();
	...
	public BSInfo parseSTND3(ParseBankStmtRequestDTO request) throws IOException{
		...
		try {
			...
			Pattern patternAccount = Pattern.compile("ACCOUNT\\s*TYPE\\s*:([\\s\\S]*?)(?=^.*?ACCOUNT\\s*NO)",
					Pattern.MULTILINE);
...
//          	e.printStackTrace();
	...
	private List<Transaction> extractTransactionsSTND_1(String pdfText, String accountNo) throws IOException {
        ...
        String[] columns = new String[7]; Arrays.fill(columns, "");
...
//            	System.out.println(singleLineArr[0]);
	...
	private List<Transaction> extractTransactionsSTND_2(String pdfText, String accountNo, String startYear) throws IOException {
		...
		pdfText = pdfText.replaceAll("Page[\\s\\S]*?domestic\\s*debit\\s*card\\s*transactions", "")
					.replaceAll("ACCOUNT\\s*STATEMENT[\\s\\S]*?\\s*Date\\s*Value\\s*Description.*\\n\\s*Date.*", "").replaceAll("\\|", "/");
	...
	private List<Transaction> extractTransactionsSTN_3(String fileName, String accountNo, String dateFormat)
			throws IOException {
		List<Transaction> transactions = new ArrayList<>();
		...
		if (txnRows != null && !txnRows.isEmpty()) {
			...
			for (String[] row : txnRows) {
				...
				if (line.contains("Total") || line.contains("Page") || line.contains("BALANCE FORWARD")
						|| data.length != 7 || headerMatcher.find() || data[4].equalsIgnoreCase("Deposit")
						|| !dateMatcher.find())
					continue;
