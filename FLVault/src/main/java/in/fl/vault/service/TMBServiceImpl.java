	...
	public BSInfo parseTMB1(ParseBankStmtRequestDTO request) throws IOException {
		...
		try {
			...
			Pattern pattern1 = Pattern.compile(
					"\\s*Statement\\s*for\\s*A/c\\s*(\\d{1,})\\s*Between\\s*([\\d]{1,}-[\\w]*-[\\w]*?)\\s*and\\s*([\\d]{1,}-[\\w]*-[\\w]*)");
			...
			Pattern pattern6 = Pattern.compile(
					"(^\\s*\\d{2}-\\d{2}-\\d{4}[\\s\\S]*?)(?=^\\s{1,14}\\d{2}-\\d{2}-\\d{4}|\\s*Page|\\s*Closing\\s*Balance)",
					Pattern.MULTILINE);
...
//	        e.printStackTrace();
	...
	public BSInfo parseTMB2(ParseBankStmtRequestDTO request) throws IOException {
		...
		try {
			...
			List<Transaction> listTransaction = new ArrayList<>();
			Pattern pattern1 = Pattern.compile(
					"Statement\\s*for\\s*account\\s*number\\s*(\\d*)\\s*Between\\s*([\\d]{1,}-[\\w]*-[\\w]*?)\\s*and\\s*([\\d]{1,}-[\\w]*-[\\w]*)");
			...
			Pattern pattern6 = Pattern.compile(
					"(^\\s*\\d{2}-\\w{3}-\\d{4}[\\s\\S]*?)(?=^\\s*\\d{2}-\\w{3}-\\d{4}|\\s*Closing\\s*Balance|\\s*Page\\s*\\d{1,}\\s*of)", Pattern.MULTILINE);
...
//          e.printStackTrace();
	...
	public BSInfo parseTMB5(ParseBankStmtRequestDTO request) {
		...
		try {
			...
			String[] period = CommonUtils.extractMultiGroupArray(pdfText,
					"for\\s*the\\s*Period.*?(\\d{2}-\\d{2}-\\d{4})\\s*to\\s*(\\d{2}-\\d{2}-\\d{4})");
			String dateFormat = "dd-MM-yyyy";
...
//        	e.printStackTrace();
	...
	public BSInfo parseTMB6(ParseBankStmtRequestDTO request) {
		...
		try {
			...
			Pattern pattern5 = Pattern.compile("IFSC\\s*Code\\s*([\\w]*)");
			Pattern pattern6 = Pattern.compile(
				    "(^\\s*\\d{2}-\\w{3}-[\\s\\S]*?)(?=^^\\s*Closing\\s*Balance|^\\s*\\d{2}-\\w{3}-)", 
				    Pattern.MULTILINE
				);
...
//			e.printStackTrace();
	...
	private List<Transaction> extractTransactionsTMB_5_(String pdfText, String accountNo) throws IOException {
		...
		Pattern datePattern = Pattern.compile(
				"(\\d{2}-\\d{2}-\\d{4})\\s*(\\d{2}-\\d{2}-\\d{4})\\s*([\\s\\S]*?)\\s+(\\.|\\d{7})\\s+([\\d,]+\\.\\d{2}|\\.)\\s+([\\d,]+\\.\\d{2}|\\.)\\s+([\\d,]+\\.\\d{2})");
