    ...
    public BSInfo parseUCO2(ParseBankStmtRequestDTO request) throws IOException {
		...
		try {
			...
			bankStatementInfo.setName(
					CommonUtils.extractField(pdfText, "\\s{25}(.*):\\s*Primary").replaceAll("\\s+", " ").trim());
			...
			bankStatementInfo.setStartDate(CommonUtils.dateFormatter(
					CommonUtils.extractField(pdfText, "FOR\\s*THE\\s*PERIOD\\s*OF\\s*(\\S*)"), dateFormat));
			bankStatementInfo.setEnDate(CommonUtils.dateFormatter(
					CommonUtils.extractField(pdfText, "FOR\\s*THE\\s*PERIOD\\s*OF.*?to\\s*(\\S*)"), dateFormat));
...
//        	(\d{2}-[A-Za-z]{3}-\d{4})\s*(.*?)\s{10,}(\S*)\s*(\S*)
			...
			bankStatementInfo.setName(
					CommonUtils.extractMultiLinesField(pdfText, "\\n(\\s*Name[\\s\\S]*?)\\n.+\\n\\s*Address", 56)
							.replace("Name", "").replaceAll("\\s+", " ").trim());
			bankStatementInfo.setIfsc(CommonUtils.extractField(pdfText, "IFSC\\s*Code\\s*(UCBA\\w{7})"));
			bankStatementInfo.setPhone1(CommonUtils.extractField(pdfText, "Mobile\\s*No.\\s*(\\S*)"));
			bankStatementInfo
					.setBranch(CommonUtils.extractField(pdfText, "Branch\\s*Name\\s*(.*)").replaceAll("\\s+", " "));
			bankStatementInfo.setAccountType(
					CommonUtils.extractField(pdfText, "A/c\\s*Type\\s*(.{30})").replaceAll("\\s+", " ").trim());
			bankStatementInfo
					.setEmail(CommonUtils.extractField(pdfText, "E-Mail\\s*ID(.{0,40})").replaceAll("\\s+", "").trim());
			bankStatementInfo.setAddress(
					CommonUtils.extractMultiLinesField(pdfText, "\\n(\\s+.*Address[\\s\\S]*?)\\n\\s*A/c", 56));
			String dateFormat = "dd-MM-yyyy";
			String txnDateFormat = "dd-MMM-yyyy";
			String[] period = CommonUtils.extractMultiGroupArray(pdfText,
					"Between\\s*(\\d{2}-\\d{2}-\\d{4}).*(\\d{2}-\\d{2}-\\d{4})");
			if (period != null) {
				bankStatementInfo.setStartDate(CommonUtils.dateFormatter(period[0], dateFormat));
				bankStatementInfo.setEnDate(CommonUtils.dateFormatter(period[1], dateFormat));
			}
			...
			if(matcher.find()) {
				...
				bankStatementInfo.setTransactions(
						extractTransactionsUCO_4_2(pdfText, bankStatementInfo.getAccountNo(), txnDateFormat));
			}else {
				log.info("extractTransactionsUCO_4_1 called");
				bankStatementInfo.setTransactions(
						extractTransactionsUCO_4_1(pdfText, bankStatementInfo.getAccountNo(), txnDateFormat));
			}
    ...
    public BSInfo parseUCO5(ParseBankStmtRequestDTO request) {
		...
		try {
			...
			bankStatementInfo.setBranch(
					CommonUtils.extractField(pdfText, "Branch\\s*:\\s*(.*)\\s*Drawing").replaceAll("\\s+", " "));
			bankStatementInfo.setName(CommonUtils.extractField(pdfText, "Name\\s*:\\s*([\\s\\S]*?)\\s*Status")
					.replaceAll("\\n", " ").replaceAll("\\s+", " ").trim());
			bankStatementInfo.setAccountType(CommonUtils.extractField(pdfText, "Type\\s*:\\s*(.*)"));
			String address1 = CommonUtils
					.extractMultiLinesField(pdfText, "Credit\\s*Accrued.*\\n([\\s\\S]*?)\\n\\s*Contact", 86)
					.replaceAll("Line\s*1\s*:", " ").trim();
			String address2 = CommonUtils
					.extractMultiLinesField(pdfText, "Credit\\s*Accrued.*\\n([\\s\\S]*?)\\n\\s*Contact", 86, 80)
					.replaceAll("Line\s*2\s*:", " ").trim();
			bankStatementInfo.setAddress((address1 + " " + address2));

			List<Transaction> listOfTransactions = extractTransactionsUCO_5(pdfText, bankStatementInfo.getAccountNo(),
					"dd/MM/yyyy");
...
//        	e.printStackTrace();
    ...
    private List<Transaction> extractTransactionsUCO_2(String text, String accountNo, String dateFormat) {
		...
		Pattern pattern = Pattern.compile(
				"^\\s*(\\d{2}-\\d{2}-\\d{4})\\s+(.{55}).{21}\\s+([\\d\\.,]+)(\\s+)([\\d\\.,]+)", Pattern.MULTILINE);
...
//		String pattern1 = "(\\d{2}-[A-Za-z]{3}-\\d{4})\\s*(\\S*)(\\s*)(\\S*)\\n([\\s\\S]*?)\\n\\s*\\d{2}-[A-Za-z]{3}-\\d{4}";
	...
	private List<Transaction> extractTransactionsUCO_5(String text, String accountNo, String dateFormat) {
		...
		Pattern pattern = Pattern.compile(
				"(\\d{2}\\/\\d{2}\\/\\d{4})\\s*(\\d{2}\\/\\d{2}\\/\\d{4})\\s*([\\d,\\.]+)(\\s+)([\\d,\\.]+)\\s*([\\s\\S]*?)(?=(\\d{2}\\/\\d{2}\\/\\d{4}) | Date)");
		Matcher matcher = pattern.matcher(text);
		int serialNoCount = 1;
